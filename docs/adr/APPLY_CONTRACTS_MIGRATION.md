...
## Quick Steps to Create the Contracts Table

Since the Supabase CLI is having sync issues, the fastest way to apply the migration is through the Supabase Dashboard.

**IMPORTANT**: This migration requires **TWO STEPS** due to PostgreSQL's requirement that enum values must be committed before use.

### Step 1: Update Enum Values

Go to: **https://supabase.com/dashboard/project/rvufhbkmqfrjdcqoeyal/sql/new**

Copy and run this SQL first (from `supabase/migrations/20250111000015_update_contract_status_enum.sql`):
...
In the same SQL Editor (or a new query), copy and run this SQL (from `supabase/migrations/20250111000016_create_contracts.sql`):

```sql
-- Create contracts table
-- This table stores rental contracts created from approved proposals

-- First, add missing enum values to contract_status if they don't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'suspended' AND enumtypid = 'contract_status'::regtype) THEN
    ALTER TYPE contract_status ADD VALUE 'suspended';
...
2. You should now see the `contracts` table
3. Refresh your app - the contracts page should now work!
...
- ✅ The `contract_status` enum will have new values: `'suspended'`, `'completed'`, `'cancelled'`
- ✅ The `contracts` table will be created with all columns and constraints
- ✅ The `proposals` table will have a new `contract_id` column
- ✅ RLS policies will be active for secure access control
- ✅ Your app's contracts page will load without errors

---

## What This Migration Does

✅ Creates the `contracts` table with all necessary columns  
✅ Adds `contract_id` column to `proposals` table  
✅ Sets up Row Level Security (RLS) policies  
✅ Creates indexes for better performance  
✅ Auto-generates contract numbers (CTR-2025-0001)  
✅ Creates triggers to:
- Auto-generate contract numbers
- Update motorcycle availability when contracts change
- Link proposals to contracts automatically
...
Your app will have:
- ✅ Working contracts page
- ✅ Ability to create contracts from approved proposals
- ✅ Contract details view
- ✅ Contract cancellation with reasons
- ✅ Automatic motorcycle availability management
